...
export function useWeb3Market() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setCharacters(chars.map((char: CharacterInterface) => ({
        id: Number(char.id),
        name: char.name,
        image: char.image,
      ...
      setHistory(historyData.map((h: PurchaseInterface) => ({
  ...
  const buyCharacter = async (id: number) => {
    ...
    try {
      ...
      const tx = await contract.buyCharacter(id, { value: ethers.parseEther("0.001") });
      await tx.wait();
  ...
  return { account, characters, history, loading, connectWallet, buyCharacter };
}